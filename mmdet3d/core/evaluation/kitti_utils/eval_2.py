...
from .rotate_iou_2 import rotate_iou_gpu_eval
...
def compute_statistics_jit(overlaps,
    ...
    if compute_fp: #something might be wrong here
        for i in range(det_size):
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        gt_datas = np.concatenate(
            ...
            [gt_annos[i]['location'], gt_annos[i]['dimensions'], gt_annos[i]['alpha'][..., np.newaxis]], 1) #ändert anscheinend gar nichts im ergebnis?
...
def do_coco_style_eval(gt_annos,
    ...
    for i in range(overlap_ranges.shape[1]):
        for j in range(overlap_ranges.shape[2]):
            #min_overlaps[:, i, j] = np.linspace(*overlap_ranges[:, i, j]) np.linspace(overlap_ranges[0, i, j], overlap_ranges[1, i, j], num=10)
...
def get_coco_eval_result(gt_annos,
    ...
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian', #Änderung: Cyclist und Pedestrian hier vertauscht
        2: 'Cyclist',
        3: 'Van',
        4: 'Person_sitting',
        5: 'car',
